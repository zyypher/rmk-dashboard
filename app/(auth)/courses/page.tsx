// Updated Courses Page with React Hook Form validation (show error messages)
...
const CoursesPage = () => {
  ...
  const handleAddOrEdit = async (data: any) => {
    ...
    try {
      ...
      if (selectedCourse) {
        await axios.put(`/api/courses/${selectedCourse.id}`, transformedData)
    ...
    <div className="space-y-6 p-6">
      ...
      >
        <div className="space-y-4">
          <div>
            <Input placeholder="Course Title" {...register("title", { required: "Course title is required" })} />
            {errors.title && <p className="text-sm text-error text-red-600 mt-1">{errors.title.message as string}</p>}
          </div>
          ...
          <div>
            <MultiSelect
              ...
              onChange={(vals: string[]) => setValue("languages", vals)}
            >
              {languagesList.map((lang) => (
                <MultiSelectItem key={lang.id} value={lang.name}>{lang.name}</MultiSelectItem>
              ))}
            </MultiSelect>
            {errors.languages && <p className="text-sm text-red-600 mt-1">At least one language is required</p>}
          </div>
          ...
          <div>
            <Select {...register("trainerId", { required: "Trainer is required" })}>
              <SelectTrigger>
            ...
            {errors.trainerId && <p className="text-sm text-red-600 mt-1">{errors.trainerId.message as string}</p>}
          </div>
      ...
      >
        <p className="text-sm">
          ...
          <span className="font-semibold text-red-600">{courseToDelete?.title}</span>?
        </p>
      </Dialog>
    </div>
  )
