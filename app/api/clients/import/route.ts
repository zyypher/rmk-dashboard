...
export async function POST(req: NextRequest) {
  try {
    ...
    const parsed = clients.map((c: any, i: number) => {
      ...
      if (!c.name || !c.phone) {
        throw new Error(`Row ${row}: Missing required fields (name or phone)`)
      }

      return {
        name: String(c.name).trim(),
        phone: String(c.phone).trim(), // Force string
        email: c.email ? String(c.email).trim() : '',
        contactPersonName: c.contactPersonName ? String(c.contactPersonName).trim() : '',
